...
public:
   ...
   * \param data Data vector, in which a part of it is to be sorted
   */
  ivec sort_index(int low, int high,Vec<T> &data);
 ...
 */
template<class T>
void sort(Vec<T> &data, SORTING_METHOD method)
{
  Sort<T> s(method);
 ...
 */
template<class T>
ivec sort_index(const Vec<T> &data, SORTING_METHOD method)
{
  Sort<T> s(method);
...
void Sort<T>::sort(int low, int high, Vec<T> &data)
{
  ...
  case INSERTSORT:
    ...
    break;
  default:
    cout<<("Sort<T>::sort(): Unknown sorting method")<<endl;abort();
    break;
  }
}
...
template<class T>
ivec Sort<T>::sort_index(int low, int high, Vec<T> &data)
{
  int N = data.size();
  // Nothing to sort if data vector has only one or zero elements
  if (N == 1)
...
//             "low or high out of bounds");
  ...
  case INSERTSORT:
    ...
    break;
  default:
    cout<<("Sort<T>::sort_index(): Unknown sorting method")<<endl;abort();
    break;
  }
...
{
  if(!(low >= 0) && (high > low) && (high < data.size()))
  {cout<<"Sort::sort(): low or high out of bounds"<<endl;abort();}
